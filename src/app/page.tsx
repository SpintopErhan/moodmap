...
export default function Home() {
  ...
  const handleAddMood = async () => {
    ...
    // Kullanıcının daha önce bir mood paylaşıp paylaşmadığını kontrol et
    ...
    if (existingMoodIndex !== -1) {
      // Eğer mevcut bir mood varsa, onu güncelle
      const existingMood = moods[existingMoodIndex];
      moodToPost = {
        ...existingMood, // Eski id'yi koru
        emoji: selectedEmoji,
        text: statusText.trim(),
        location: newLocation,
        locationLabel: newLocationLabel,
        timestamp: Date.now(),
        // userId ve username zaten mevcut olduğu için değişmez
      };
      ...
      // Eğer mevcut bir mood yoksa, yeni bir mood oluştur
      moodToPost = {
          id: Math.random().toString(36).substr(2, 9), // Yeni mood için yeni ID
          emoji: selectedEmoji,
          text: statusText.trim(),
          location: newLocation,
          locationLabel: newLocationLabel,
      ...
      setMoods(prev => [moodToPost, ...(prev || [])]); // Yeni mood'u en başa ekle
    }
    
    setView(ViewState.MAP); // Mood post edildikten sonra haritaya dön
    setStatusText('');
    setIsSubmitting(false);

    // Farcaster'da da paylaşım
    if (user?.fid) { // Sadece Farcaster kullanıcısı bağlıysa
      try {
        const castContent = moodToPost.text ? `${moodToPost.emoji} ${moodToPost.text} at ${moodToPost.locationLabel || "a location"}` : `${moodToPost.emoji} at ${moodToPost.locationLabel || "a location"}`;
  ...
  // --- Drag Scroll Handlers for Emoji Picker ---
  ...
  // Farcaster SDK yükleme/hata durumları için genel UI
  if (status === "loading") {
    return (
    ...
    <div className="relative w-full h-screen flex flex-col bg-slate-950 text-white overflow-hidden font-sans">
      ...
      <div className="flex-1 relative z-0">
        <div className="absolute inset-0">
            <DynamicMap moods={moods} onInitialLocationDetermined={handleInitialLocationDetermined} height="100%" />
        </div>
